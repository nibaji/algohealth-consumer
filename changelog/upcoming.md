...
- 2026-06-17: Added ability to attach new audio recordings and additional documents when editing an existing medical record. The edit mode now shows an "Add New Attachments" section with a document picker (multi-select) and an `AudioNoteRecorder`. On save, new attachments are uploaded via a two-step flow (POST `/utils/store-files` then POST `/documents/upload` with `medical_record_id`) before the JSON PATCH is applied. Staged files are cleared on save or cancel. Changed in `app/medical-records/[id].tsx`, `src/services/medical-records/medicalRecordService.ts`.
