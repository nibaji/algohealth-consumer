...
- 2026-06-17: Implemented retrieval, playback, and download support for audio notes and document attachments in the Medical Record Details screen. Integrated a custom `fileService` using `expo-file-system/legacy` to pull secure items via the `/utils/get-file` POST endpoint and cache them locally, playing back audio with `AudioPlayerView` and downloading documents via dynamic form submission in the browser. Changed in `app/medical-records/[id].tsx`, `src/features/medical-records/medicalRecordTypes.ts`, `src/services/medical-records/fileService.ts`.
